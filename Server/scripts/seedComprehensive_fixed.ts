...
async function seedComprehensive() {
  ...
  try {
    ...
    for (const date of availabilityDates) {
      ...
      if (!existing) {
        const reviewedAt = new Date(date.getTime() + 2 * 60 * 60 * 1000); // 2 hours after submission
        ...
        await logAuditEvent({
          ...
          metadata: { date: date.toISOString(), status: "APPROVED" },
        });
      }
    }
    ...
    // Completed visits (5-10)
    for (let i = 0; i < 7; i++) {
      const scheduledAt = randomDateInPast(25);
      const createdAt = new Date(scheduledAt.getTime() - 2 * 24 * 60 * 60 * 1000); // Created 2 days before
      const completedAt = new Date(scheduledAt.getTime() + 60 * 60 * 1000); // Completed 1 hour after scheduled
      ...
      // Audit logs for visit lifecycle
      await logAuditEvent({
        actorId: patient.id,
        ...
        metadata: { scheduledAt: scheduledAt.toISOString(), visitType: visit.visitType },
        createdAt: createdAt,
      });
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const cancelledAt = new Date(scheduledAt.getTime() - 1 * 24 * 60 * 60 * 1000); // Cancelled 1 day before
      ...
      await logAuditEvent({
        ...
        metadata: { reason: visit.cancelReason },
        createdAt: cancelledAt,
      });
    }

    // Confirmed visits (upcoming, 2-3)
    for (let i = 0; i < 3; i++) {
    ...
    // Requested visits (pending, 1)
    const requestedScheduledAt = randomDateInFuture(10);
    const requestedCreatedAt = new Date(Date.now() - 1 * 60 * 60 * 1000); // 1 hour ago
    ...
    await logAuditEvent({
      ...
      metadata: { scheduledAt: requestedScheduledAt.toISOString() },
      createdAt: requestedCreatedAt,
    });
    ...
    for (const med of medications) {
      ...
      await logAuditEvent({
        ...
        metadata: { medicationName: med.name },
        createdAt: startDate,
      });
    }
    ...
    await logAuditEvent({
      ...
      metadata: { subject: "Question about medication", recipientIds: [clinician.id] },
      createdAt: conv1CreatedAt,
    });

    const messages1 = [
      { senderId: patient.id, content: "Hi, I have a question about my Metformin dosage. Should I take it before or after meals?", delay: 0 },
      { senderId: clinician.id, content: "Hello! Metformin should be taken with meals to reduce stomach upset. Take it at the same time each day.", delay: 2 },
      { senderId: patient.id, content: "Thank you! That's very helpful.", delay: 5 },
    ];
    ...
    for (const msg of messages1) {
      ...
      await logAuditEvent({
        ...
        actorRole: msg.senderId === patient.id ? patient.role : clinician.role,
        actionType: "MESSAGE_SENT",
        targetType: "Message",
        targetId: conversation1.id,
        description: `Message sent in conversation`,
        metadata: { conversationId: conversation1.id, messageLength: msg.content.length, isReply: msg.delay > 0 },
        createdAt: msgCreatedAt,
      });
    }
    ...
    await logAuditEvent({
      ...
      metadata: { subject: "Upcoming appointment", recipientIds: [caregiver.id] },
      createdAt: conv2CreatedAt,
    });

    const messages2 = [
      { senderId: patient.id, content: "I have an appointment next week. Can you help me prepare?", delay: 0 },
      { senderId: caregiver.id, content: "Of course! What day is it scheduled for?", delay: 1 },
      { senderId: patient.id, content: "It's on Thursday at 2 PM.", delay: 3 },
      { senderId: caregiver.id, content: "Perfect, I'll make sure we're ready. I'll help you gather your medication list.", delay: 4 },
    ];
    ...
    for (const msg of messages2) {
      ...
      await logAuditEvent({
        actorId: msg.senderId,
        actorRole: msg.senderId === patient.id ? patient.role : caregiver.role,
        actionType: "MESSAGE_SENT",
        targetType: "Message",
        targetId: conversation2.id,
        description: `Message sent in conversation`,
        metadata: { conversationId: conversation2.id, messageLength: msg.content.length, isReply: msg.delay > 0 },
        createdAt: msgCreatedAt,
      });
    }
    ...
    if (!existingLink) {
      ...
      await logAuditEvent({
        ...
        metadata: { email: caregiver.email, code: invitation.code },
        createdAt: invitationCreatedAt,
      });
      ...
      await logAuditEvent({
        ...
        metadata: { relationship: "MPOA", invitationCode: invitation.code },
        createdAt: linkCreatedAt,
      });
    ...
    for (const visit of feedbackVisits) {
      const feedbackCreatedAt = new Date(visit.completedAt!.getTime() + 24 * 60 * 60 * 1000); // 1 day after visit
      await prisma.familyFeedback.create({
        data: {
          ...
          relatedId: visit.id,
          ratingHelpfulness: 4 + Math.floor(Math.random() * 2), // 4 or 5
    ...
    for (let day = 30; day >= 0; day--) {
      ...
      // Random users login each day (not all users every day)
      const numLogins = 1 + Math.floor(Math.random() * 3); // 1-3 users per day
      ...
      for (let i = 0; i < numLogins && i < shuffled.length; i++) {
        ...
        await logAuditEvent({
          ...
          metadata: { success: true, rememberMe: false },
          createdAt: loginDate,
        });
        loginCount++;
      }
