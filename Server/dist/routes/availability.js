...
router.get("/", async (req, res) => {
    try {
        ...
        if (user.role === "CLINICIAN") {
            ...
            where.status = client_1.AvailabilityStatus.APPROVED;
        }
...
router.get("/:id", async (req, res) => {
    try {
        const user = getUser(req);
        const record = await db_1.prisma.clinicianAvailability.findUnique({
        ...
        if (user.role === "PATIENT") {
            if (record.status !== client_1.AvailabilityStatus.APPROVED) {
                return res.status(403).json({ error: "Forbidden" });
            }
...
router.post("/", async (req, res) => {
    try {
        ...
        if (user.role === "ADMIN" && bodyClinicianId) {
            const clinician = await db_1.prisma.user.findUnique({
        ...
        const availability = await db_1.prisma.clinicianAvailability.upsert({
            where: {
                clinicianId_date: {
...
router.post("/batch", async (req, res) => {
    try {
        ...
        const results = await db_1.prisma.$transaction(days.map((day) => db_1.prisma.clinicianAvailability.upsert({
            where: {
                clinicianId_date: {
                    clinicianId: targetClinicianId,
...
router.patch("/:id/review", requireRole_1.requireAdmin, async (req, res) => {
    try {
        ...
        const reviewable = [
            client_1.AvailabilityStatus.APPROVED,
            client_1.AvailabilityStatus.REJECTED,
        ];
        if (!reviewable.includes(status)) {
            return res.status(400).json({ error: "status must be APPROVED or REJECTED" });
        }
        if (record.status !== client_1.AvailabilityStatus.PENDING) {
            return res.status(400).json({
                error: `Only PENDING records can be reviewed. Current status: ${record.status}`,
            });
        }
        const availability = await db_1.prisma.clinicianAvailability.update({
            where: { id: req.params.id },
            data: {
                status: status,
                reviewedBy: admin.id,
...
router.delete("/:id", async (req, res) => {
    try {
        const user = getUser(req);
        const existing = await db_1.prisma.clinicianAvailability.findUnique({
            where: { id: req.params.id },
        ...
        if (user.role === "CLINICIAN") {
            ...
            if (existing.status !== client_1.AvailabilityStatus.PENDING) {
                return res.status(400).json({
        ...
        await db_1.prisma.clinicianAvailability.delete({ where: { id: req.params.id } });
        res.json({ ok: true });
    }
    catch (e) {
