...
async function verifyLink(caregiverId, patientId) {
    return db_1.prisma.caregiverPatientLink.findFirst({
...
router.get("/:patientId/overview", async (req, res) => {
    try {
        ...
        const [patientUser, upcomingVisits, recentCompletedVisits, missedVisits, medications,] = await Promise.all([
        ...
        for (const med of medications) {
            if (med.refillDueDate) {
                const daysUntil = Math.ceil((new Date(med.refillDueDate).getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
