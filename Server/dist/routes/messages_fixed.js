...
router.get("/assigned-clinicians", async (req, res) => {
    try {
        ...
        const clinicians = assignments.map((a) => ({
            id: a.clinician.id,
...
router.get("/notifications", async (req, res) => {
    try {
        ...
        const notifications = participants.map((participant) => {
            ...
            const otherParticipant = participant.conversation.participants.find((p) => p.userId !== user.uid);
...
router.post("/debug/create-assignment", async (req, res) => {
    try {
...
router.get("/debug/participants", async (req, res) => {
    try {
        ...
        let assignments = [];
        if (user.role === "PATIENT") {
            assignments = await db_1.prisma.patientAssignment.findMany({
...
router.post("/debug/send-test-message", async (req, res) => {
    try {
...
router.delete("/:id", async (req, res) => {
    try {
        ...
        const messageId = req.params.id;
        await db_1.prisma.message.delete({
            where: { id: messageId }
        });
