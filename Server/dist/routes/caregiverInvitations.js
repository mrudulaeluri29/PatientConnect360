...
router.post("/", requireAuth_1.requireAuth, async (req, res) => {
    try {
        ...
        do {
            code = generateCode();
            const existing = await db_1.prisma.caregiverInvitation.findUnique({ where: { code } });
            if (!existing)
                break;
            attempts++;
        } while (attempts < 5);
        ...
        const invitation = await db_1.prisma.caregiverInvitation.create({
            data: {
                patientId: user.id,
                code,
...
router.get("/", requireAuth_1.requireAuth, async (req, res) => {
    try {
        ...
        if (statusParam && Object.values(client_1.InvitationStatus).includes(statusParam)) {
            where.status = statusParam;
        }
        // Auto-expire any PENDING invitations past their expiresAt
        await db_1.prisma.caregiverInvitation.updateMany({
            where: {
                patientId: user.id,
                status: "PENDING",
        ...
        const invitations = await db_1.prisma.caregiverInvitation.findMany({
            where,
...
router.delete("/:id", requireAuth_1.requireAuth, async (req, res) => {
    try {
        ...
        const updated = await db_1.prisma.caregiverInvitation.update({
            where: { id: req.params.id },
            data: { status: "REVOKED" },
            select: invitationSelect,
        });
