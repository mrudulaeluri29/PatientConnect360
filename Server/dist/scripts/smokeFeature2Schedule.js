...
async function main() {
    ...
    const notifs = await db_1.prisma.notification.findMany({
        take: 5,
    ...
    const prefs = await db_1.prisma.visitReminderPreference.findMany({ take: 5 });
    console.log(`\n✅ Reminder preferences in DB: ${prefs.length}`);
    prefs.forEach((p) => {
        console.log(`   - userId: ${p.userId} | channel: ${p.channel} | enabled: ${p.enabled}`);
    });
    const reminders = await db_1.prisma.outboundNotification.findMany({
        take: 5,
        orderBy: { sendAt: "asc" },
    });
