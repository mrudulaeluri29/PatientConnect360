...
(0, apiTestClient_1.describeWithRoleCredentials)("Feature 4 documentation gating", ["patient", "clinician", "admin"], () => {
    let patient;
    let clinician;
    let admin;
    let visitId = "";
    ...
    it("allows completion with adequate notes (>= 50 chars)", async () => {
        const longNotes = "Patient tolerated treatment well. Vitals stable, gait training completed, medication questions answered, and follow-up plan reviewed.";
        const res = await (0, supertest_1.default)(apiTestClient_1.BASE_URL)
            .patch(`/api/visits/${visitId}`)
            .set((0, apiTestClient_1.authed)(clinician))
    ...
    it("completed visit cannot be re-completed", async () => {
        const anotherNote = "Attempting to re-complete a visit that was already finalized — this should fail or be idempotent.";
        const res = await (0, supertest_1.default)(apiTestClient_1.BASE_URL)
            .patch(`/api/visits/${visitId}`)
            .set((0, apiTestClient_1.authed)(clinician))
