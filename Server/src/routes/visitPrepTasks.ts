...
router.get("/:visitId/prep-tasks", async (req: Request, res: Response) => {
  try {
...
// Body: { text }
router.post("/:visitId/prep-tasks", async (req: Request, res: Response) => {
  try {
    const user = getUser(req);
    ...
    const task = await prisma.visitPrepTask.create({
      data: {
        visitId,
...
// Body: { isDone?, text? }
router.patch("/prep-tasks/:taskId", async (req: Request, res: Response) => {
  try {
    const user = getUser(req);
    const { taskId } = req.params;

    const task = await prisma.visitPrepTask.findUnique({
      where: { id: taskId },
    ...
    const updated = await prisma.visitPrepTask.update({
      where: { id: taskId },
      data,
      include: {
        createdByClinician: { select: { id: true, username: true } },
