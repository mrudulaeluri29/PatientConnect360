  ...
  try {
    ...
    if (user.role === "PATIENT") {
      ...
      const assignment = await prisma.patientAssignment.findFirst({
        where: { clinicianId: user.id, patientId: patientId as string, isActive: true },
      });
      ...
      const link = await prisma.caregiverPatientLink.findFirst({
        where: { caregiverId: user.id, patientId: patientId as string, isActive: true },
      });
      if (!link) return res.status(403).json({ error: "Not linked to this patient" });
      targetPatientId = patientId as string;
    } else if (user.role === "ADMIN") {
      targetPatientId = patientId as string | undefined;
    }
...
router.post("/assignments", async (req: Request, res: Response) => {
  try {
    ...
    const exerciseAssignment = await prisma.exerciseAssignment.create({
      data: {
        patientId,
...
router.patch("/assignments/:id", async (req: Request, res: Response) => {
  try {
    ...
    const existing = await prisma.exerciseAssignment.findUnique({
