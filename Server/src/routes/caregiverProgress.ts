...
type GoalStatus = "on_track" | "attention" | "risk";

function goalStatusFromPercent(pct: number, empty: boolean): GoalStatus {
  if (empty) return "attention";
...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    const upcomingTerminal: VisitStatus[] = [
    ...
    const patients = await Promise.all(
      links.map(async (link) => {
        ...
        const completedCount = countCompletedVisitsInRollingWindow(pVisits, 30, now);
        const missedCount = countMissedVisitsInRollingWindow(pVisits, 30, now);
        const upcomingCount = countUpcomingVisits(pVisits);
        ...
        const privacy = await getPatientPrivacySettings(pId);
        const carePlanAllowed = privacy.carePlanVisibleToCaregivers;
        const primaryPlan = carePlanAllowed ? await loadPrimaryPlanForTherapy(pId) : null;
        const therapy = await buildTherapyProgressOverview({
          patientId: pId,
          carePlanAllowed,
          primaryPlan,
        });
        ...
        const goals = [
          carePlanAllowed
            ...
            : {
                ...
                progress: 0,
                status: "attention" as GoalStatus,
              },
          {
            id: `${pId}-goal-hep`,
            title: "Therapy exercise adherence",
            target:
              therapy.hep.expectedCompletionsThisWeek > 0
