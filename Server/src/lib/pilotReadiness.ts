...
import { buildAdminAnalytics } from "./adminKpis";

export type PilotChecklistStatus = "complete" | "attention" | "monitor";
...
export async function buildPilotReadiness() {
  const [settings, analytics, adminCount, patientCount, clinicianCount, loginAuditCount] = await Promise.all([
    ...
    prisma.auditLog.count({ where: { actionType: "LOGIN" } }),
  ]);
  ...
  const visitWorkflowReady =
    analytics.summary.visitsPerWeek > 0 ||
  ...
  const checklist: PilotChecklistItem[] = [
    makeItem(
      "admin-coverage",
      "Access",
      "Admin access is provisioned",
      adminCoverageReady ? "complete" : "attention",
      adminCoverageReady ? `${adminCount} admin account(s) can access the operational console.` : "Create at least one admin user for pilot oversight.",
      true
    ),
    makeItem(
      "user-base",
      "Onboarding",
      "Pilot users are present",
      userBaseReady ? "complete" : "attention",
      userBaseReady
        ? `${patientCount} patient(s) and ${clinicianCount} clinician(s) are available for pilot workflows.`
        : "Add at least one patient and one clinician before pilot validation.",
      true
    ),
    makeItem(
      "assignments",
      "Operations",
      "Patient-clinician assignments exist",
      activeAssignmentsReady ? "complete" : "attention",
      activeAssignmentsReady
        ? `${analytics.operationalQueues.activeAssignments} active assignment(s) are in place.`
        : "Create at least one patient-clinician assignment.",
      true
    ),
    makeItem(
      "branding",
      "Configuration",
      "Branding and support contact are configured",
      brandingConfigured && supportConfigured ? "complete" : "attention",
      brandingConfigured && supportConfigured
        ? "Portal branding and support contact details are set for pilot users."
        : "Finish branding/support details so agency users know who to contact during the pilot.",
      true
    ),
    makeItem(
      "messaging",
      "Features",
      "Messaging is enabled for the pilot",
      settings.messagingEnabled ? "complete" : "attention",
    ...
    makeItem(
      "records",
      "Features",
      "Records and documentation access is enabled",
      settings.recordsEnabled ? "complete" : "attention",
    ...
    makeItem(
      "feedback",
      "Quality",
      "Family feedback capture is ready",
      feedbackObservabilityReady ? "complete" : "monitor",
      feedbackObservabilityReady
    ...
    makeItem(
      "visit-workflow",
      "Operations",
      "Visit workflow is active",
      visitWorkflowReady ? "complete" : "monitor",
      visitWorkflowReady
    ...
    makeItem(
      "audit",
      "Governance",
      "Audit logging is recording activity",
      auditReady ? "complete" : "attention",
      auditReady ? `${loginAuditCount} login audit event(s) detected.` : "No login audit events found yet; verify authentication and audit capture.",
      true
    ),
  ];
